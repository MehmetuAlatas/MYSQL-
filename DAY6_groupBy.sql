...
select urun_adi, count(isim) toplam_urun
from manav 
...
ORDER BY isim DESC;
 ...
 -- aggragate fonksiyonlari ile ilgili bir koşul koymak için group by dan sonra having cümleciği kullanilir.
 ...
 from manav
 group by urun_adi
 having max!=min(urun_miktari) -- != yerine <> kullanilabilir
 order by max;
